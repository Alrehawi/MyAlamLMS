...
$session->check_permission('ModuleTranslate', '../');
global $database;
...
// Do Edit Action
if (check_var("edit", "POST") && check_var("check", "POST") && $session->check_permission('ModuleTranslate', '_manage.php')) {
    return $Action->do_action('edit', $_POST['check'], '_translate_edit.php', FALSE);
}

// Do Delete Action
if (check_var("dell", "POST") && check_var("check", "POST") && $session->check_permission('ModuleTranslate', '_manage.php')) {
...
if (empty($_GET['parent']) || !isset($_GET['parent'])) {
    ...
    $parent = Module::find_by_id($_GET['parent']," AND site_id={$session->site_id}");
}
...
if (isset($_POST['submit'])) {
  if(!checkToken($_POST['_token'])){
    ...
    if (!empty($title)) {
        //check lang added
        if (Translator::check_lang($parent->id, $lang_id, Module::$trans_key) > 0) {
        ...
        foreach ($fields as $key => $value):
            $new_translate = Translator::translate($parent->id, $key, $value, Module::$trans_key, $lang_id);
                    ...
                    <table width="100%" class="table table-bordered table-striped table-hover  first-table" id="dataTables-example">
                        <thead>
                             <tr>
                                ...
                                <th width='80'><?php echo read_xmls('/site/lang/titles/main') ?></th>
                        ...
                        <tbody>
                               <?php
                                    $translates = Translator::find_translate_item($parent->id, Module::$trans_key, 'title');
                                    foreach ($translates as $translate):
                                        ...
                                        <tr>
                                            ...
                                            <td width='80' align="center"><input type="checkbox" value="<?php echo $translate->id; ?>" name="check[]" title="<?php echo $translate->content; ?>" <?php
                                                if ((is_array($checked_row) && in_array($translate->id, $checked_row)) || check_var("checked_row", "GET") == $translate->id) {
                                                    echo "checked='checked'";
                                                }
                                                ?>/>
                                                <input type="hidden" name="item_lang" value="<?php echo $translate->lang_id ?>" /></td>
                                        </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
