...
$session->check_permission('UserDelete', '_manage.php');
...
if (!is_array($checked_row)) {
    ...
    for ($i = 0; $i < $count_array; $i++) {
        ...
        $user = User::find_by_id($checked_row[$i]);
        $user_admin = User::find_by_id($session->user_id);

        if ($user && $user->delete()) {
            $msg[] = ("{$user->username} " . read_xmls('/site/msg/wesdel'));
            echo log_action("Delete user {$user->username} - id num : ({$user->id}) ", "By: {$user_admin->username} and Related Profile");
        } else {
