...
$session->check_permission('PermissionDelete', '_manage.php');
...
if (!is_array($checked_row)) {
    ...
    for ($i = 0; $i < $count_array; $i++) {
        ...
        $per = Permission::find_by_id($checked_row[$i]);
        $user_admin = User::find_by_id($session->user_id);

        if ($per && $per->delete()) {
            $msg[] = ("{$per->title} " . read_xmls('/site/msg/wesdel'));
            echo log_action("Delete Permission: {$per->title} - id num : ({$per->id}) ", "By: {$user_admin->username}");
        } else {
