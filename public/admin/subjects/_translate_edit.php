...
$session->check_permission('SubjectTranslate', '_manage.php');
?>
<?php
...
$parent = Subject::find_by_id($cur_trans->parent_id);
...
if (isset($_POST['submit'])) {
  if(!checkToken($_POST['_token'])){
    ...
    $content_short = trim($_POST['content_short']);
...
$translate_short = Translator::find_translate_by_parent_lang_type($cur_trans->parent_id, $cur_trans->item_type, 'content_short', $cur_trans->lang_id);
    ...
    <div class="col-lg-12">
        <div class="panel panel-default" style="border: none;">
            <div  class="panel-heading">
                <h2><?php echo read_xmls('/site/subject/titles/edit') ?>: <?php echo $translate[0]->content; ?></h2>
                <?php if ($session->has_permission('SubjectAdd')) { ?>
                    <a  class=" btn btn-primary pull-left  margin-link" href="_add.php"><?php echo read_xmls('/site/subject/titles/add') ?><i class="fa fa-plus margin-right-fivePx" aria-hidden="true"></i></a>
                <?php } ?>
                <?php if ($session->has_permission('SubjectView')) { ?>
                    <a  class=" btn btn-primary pull-left  margin-link" href="_manage.php"><?php echo read_xmls('/site/subject/titles/manage') ?><i class="fa fa-th-list margin-right-fivePx" aria-hidden="true"></i></a>
                    <?php } ?>
                <a  class=" btn btn-info pull-left  margin-link" href="_translate.php?parent=<?php echo $cur_trans->parent_id; ?>">
                <?php echo read_xmls('/site/subject/lables/translate') . ": " . $parent->title; ?><i class="fa fa-language margin-right-fivePx" aria-hidden="true"></i>
                </a>
            </div>
            <div class="panel-body">
                <div class="row">
                    <div class="col-lg-12">
                       <form name="subject" action="_translate_edit.php?id=<?php echo $cur_trans->id; ?>" method="POST" enctype="multisubject/form-data">
                         <?php echo setToken() ?>
