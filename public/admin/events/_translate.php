...
$session->check_permission('EventTranslate' , '../');
global $database;
...
// Do Edit Action
	if(check_var("edit" , "POST") && check_var("check" , "POST") && $session->check_permission('EventTranslate' , '_manage.php')){
		return $Action->do_action( 'edit' , $_POST['check'] , '_translate_edit.php' , FALSE);
	}
// Do Delete Action
	if(check_var("dell" , "POST") && check_var("check" , "POST") && $session->check_permission('EventTranslate' , '_manage.php')){
...
if(empty($_GET['parent']) || !isset($_GET['parent'])){
	...
	$parent=Event::find_by_id($_GET['parent']," AND site_id={$session->site_id}");
}
...
if(isset($_POST['submit'])){
	...
	if(!empty($title) && !empty($location)){
		//check lang added
		if(Translator::check_lang($parent->id , $lang_id , Event::$trans_key)>0){
		...
		foreach($fields as $key => $value):
			$new_translate = Translator::translate($parent->id , $key , $value , Event::$trans_key , $lang_id);
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                    <?php if($session->has_permission('EventAdd')){?>
                       <a class=" btn btn-primary" href="_add.php"><?php echo read_xmls('/site/event/titles/add')?><i class="fa fa-plus" aria-hidden="true"></i></a>
                    <?php }?>

                    <?php if($session->has_permission('EventView')){?>
                        <a class=" btn btn-primary" href="_manage.php"><?php echo read_xmls('/site/event/titles/manage')?><i class="fa fa-th-list margin-right-fivePx" aria-hidden="true"></i></a>
                    <?php }?>
            </div>
            <!-- /.panel-heading -->
            <div class="panel-body">
                <form action="<?php echo get_current_page(); ?>" method="POST">
                    ...
                    <table width="100%" class="table table-bordered table-striped table-hover first-table" id="dataTables-example">
                        ...
                        <tbody>
                          <?php
                              $translates = Translator::find_translate_item($parent->id , Event::$trans_key , 'title');
                          ...
                          <tr>
                            ...
                            <input type="checkbox" value="<?php echo $translate->id; ?>" name="check[]" title="<?php echo $translate->content;?>" <?php if( (is_array($checked_row) && in_array($translate->id ,$checked_row)) || check_var("checked_row" , "GET") == $translate->id){ echo "checked='checked'";}?>/>
...
<div class="row">
    <div class="col-lg-12">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h2><?php echo read_xmls('/site/event/lables/translate').": ".$parent->title?> </h2>
