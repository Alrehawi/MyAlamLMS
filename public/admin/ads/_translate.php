...
$session->check_permission('AdTranslate', '../');
global $database;
...
// Do Edit Action
if (check_var("edit", "POST") && check_var("check", "POST") && $session->check_permission('AdTranslate', '_manage.php')) {
    return $Action->do_action('edit', $_POST['check'], '_translate_edit.php', FALSE);
}
// Do Delete Action
if (check_var("dell", "POST") && check_var("check", "POST") && $session->check_permission('AdTranslate', '_manage.php')) {
...
if (empty($_GET['parent']) || !isset($_GET['parent'])) {
    ...
    $parent = Ad::find_by_id($_GET['parent']);
}
...
if (isset($_POST['submit'])) {
    ...
    if (!empty($title)) {
        //check lang added
        if (Translator::check_lang($parent->id, $lang_id, Ad::$trans_key) > 0) {
        ...
        foreach ($fields as $key => $value):
            $new_translate = Translator::translate($parent->id, $key, $value, Ad::$trans_key, $lang_id);
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                    <?php if ($session->has_permission('AdAdd')) { ?>
                        <a  class=" btn btn-primary"  href="_add.php"><?php echo read_xmls('/site/ad/titles/add') ?><i class="fa fa-plus" aria-hidden="true"></i></a>
                    <?php } ?>
                    <?php if ($session->has_permission('AdView')) { ?>
                        <a  class=" btn btn-primary"  href="_manage.php"><?php echo read_xmls('/site/ad/titles/manage') ?><i class="fa fa-th-list margin-right-fivePx" aria-hidden="true"></i></a>
                    <?php } ?>
                ...
                <form action="<?php echo get_current_page(); ?>" method="POST">
                  <?php echo $token_input ?>
                    <table width="100%" class="table table-bordered table-striped table-hover first-table" id="dataTables-example">
                        ...
                        <tbody>
                            <?php
                            $translates = Translator::find_translate_item($parent->id, Ad::$trans_key, 'title');
...
<div class="row">
    <div class="col-lg-12">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h2><?php echo read_xmls('/site/ad/lables/translate') . ": " . $parent->title ?> </h2>
