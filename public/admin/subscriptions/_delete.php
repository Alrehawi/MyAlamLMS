...
$session->check_permission('SubscriptionDelete', '_manage.php');
...
if (!is_array($checked_row)) {
    ...
    for ($i = 0; $i < $count_array; $i++) {
        ...
        $subscription = Subscription::find_by_id($checked_row[$i]);
        $user_admin = User::find_by_id($session->user_id);

        // Delete Subscription(s) other languages
        if ($subscription && $subscription->delete()) {
            $msg[] = ("{$subscription->full_name} " . read_xmls('/site/msg/wesdel'));
            echo log_action("Delete Subscription {$subscription->full_name} - id num : ({$subscription->id}) ", "By: {$user_admin->username}");
        } else {
