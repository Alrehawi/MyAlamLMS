...
$session->check_permission('VoteAnswerTranslate', '../');
global $database;
...
// Do Edit Action
if (check_var("edit", "POST") && check_var("check", "POST") && $session->check_permission('VoteAnswerTranslate', '_manage.php')) {
    return $Action->do_action('edit', $_POST['check'], '_translate_edit.php', FALSE);
}
// Do Delete Action
if (check_var("dell", "POST") && check_var("check", "POST") && $session->check_permission('VoteAnswerTranslate', '_manage.php')) {
...
if (empty($_GET['parent']) || !isset($_GET['parent'])) {
    ...
    $parent = VoteAnswer::find_by_id($_GET['parent']);
}
...
if (isset($_POST['submit'])) {
  if(!checkToken($_POST['_token'])){
    ...
    if (!empty($title)) {
        //check lang added
        if (Translator::check_lang($parent->id, $lang_id, VoteAnswer::$trans_key) > 0) {
        ...
        foreach ($fields as $key => $value):
            $new_translate = Translator::translate($parent->id, $key, $value, VoteAnswer::$trans_key, $lang_id);
