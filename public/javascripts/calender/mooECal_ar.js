...
var Calendar = new Class({
    ...
    initialize: function (options) {
        ...
        this.extendDate();
        this.days = ['الأحد', 'الأثنين', 'الثلاثاء', 'الأربعاء', 'الخميس', 'الجمعه', 'السبت'];
        this.months = ['يناير', 'فبراير', 'مارس', 'ابريل', 'مايو', 'يونيه', 'يوليو', 'أغسطس', 'سبتمبر', 'أكتوبر', 'نوفمبر', 'ديسمبر'];
    ...
    showControlsRow: function (calTitle) {
        ...
        var aNextCal = new Element('a', {
            ...
            'class': 'aNextCal'
        }).set('html', '&rsaquo;').inject(liNextCal);
        ...
        var aPrevCal = new Element('a', {
            'href': 'javascript:void(0)',
            'class': 'aPrevCal'
        }).set('html', '&lsaquo;').inject(liPrevCal);
        ...
        }).set('text', 'شهري').inject(liMonthPicker);
        ...
        }).set('text', 'اسبوعي').inject(liWeekPicker);
        ...
        }).set('text', 'يومي').inject(liDayPicker);
    ...
    showMonth: function () {
        JJJ(this.options.calContainer).set('html', '');
            ...
            for (var j = 0; j < 7; j++) {
                ...
                if (day > 0 && day <= this.daysInMonth) {
                    ...
                    td.addEvent('click', function () {
                        JJJ('td.monthDay').each(function (td) {
        ...
        table.inject(JJJ(this.options.calContainer));
        this.getCalEvents();
    ...
    showWeek: function () {
        ...
        for (var i = 0; i < 7; i++) {
            ...
            td.addEvent('click', function () {
                JJJ('td.weekDay').each(function (td) {
                    td.removeClass('selected')
                });
                this.addClass('selected');
            });
    ...
    loadCalEvents: function () {
        var time = '';
        JJJJJJ('div.tip').each(function (divs) {
            divs.getParent().destroy();
        }); // Tips cleanup - tip divs kept building up after switching calendar views
            ...
            if ((eStart >= this.viewStartDate && eStart <= this.viewEndDate) || (eEnd >= this.viewStartDate && eEnd <= this.viewEndDate)) {
                while (eStart <= eEnd) {
                    if (eStart >= this.viewStartDate && eStart <= this.viewEndDate) {
                        ...
                        }).store('date', eStart.getDate()).inject(JJJ('day' + eStart.getDate()).getChildren('div')[0]);
                        if (this.options.view != 'month') // the month view only shows a portion of the event description
                            ...
                            if (this.options.scroller)
                                new Scroller(JJJ('day' + eStart.getDate()).getChildren('div')[0], {
                                    'area': 20
