...
var popUpCal = {
    ...
    yearRange: '-20:+10', // Range of years to display in drop-down, either relative to current year (-nn:+nn) or absolute (nnnn:nnnn)
    ...
    parseDate: function (Text) {
        ...
        if (this.timeSeparators && this.timeSeparators.length > 1) {
            ...
            if (currentTimeAndDate.length > 0)
                var currentDate = currentTimeAndDate[0].split(this.dateFormat.charAt(3));
            else
                var currentDate = [];
            if (currentTimeAndDate.length > 1)
                var currentTime = currentTimeAndDate[1].split(this.timeSeparators[1]);
            else
                var currentTime = [];
        }
        else {
    ...
    showCalendar: function () {
        ...
        if (this.timeSeparators && this.timeSeparators.length > 1) {
            ...
            if (this.timeSeparators.length == 4) {
                ...
                for (var a = 1; a < 12; a++) {
                    if (this.selectedHour == a || (a == 12 && this.selectedHour == 0) || (a + 12 == this.selectedHour))
                        timeSelect += '<option value="' + a + '" selected="selected">' + a + '</option>';
                    else
                        timeSelect += '<option value="' + a + '">' + a + '</option>';
                }
            } else {
                for (var a = 0; a < 24; a++) {
        ...
        if (this.timeSeparators && this.timeSeparators.length > 1) {
            $('#calendar_hour').change(function () {	//change hour
                ...
                if (popUpCal.timeSeparators.length == 4) {
                    ...
                    if (ampm == 'AM' && popUpCal.selectedHour == 12)
